...
A small terminal text editor, written in and scriptable with
[let-go](https://github.com/nooga/let-go) — the same Clojure-dialect Lisp
that runs the editor itself. There's no separate plugin language: every
built-in command and key binding is defined the same way a user's config
file defines new ones.
...
This installs a self-contained native binary — no separate `lg` runtime
needed. Just run `legmacs [file...]`.

### Prebuilt binaries

Grab a tarball for your platform from the
[latest release](https://github.com/nooga/legmacs/releases/latest)
(`darwin`/`linux`, `amd64`/`arm64`), extract, and put `legmacs` on your
`PATH`. These are the AOT-compiled native builds produced by `build.sh`.

### From source (interpreted)

Requires `lg` (the [let-go](https://github.com/nooga/let-go) runtime) on
`PATH`. Run:
...
from this directory, or install the `bin/legmacs` wrapper somewhere on your
`PATH` (or symlink to it) to run it from anywhere:
...
`./build.sh` AOT-compiles the whole editor into one standalone binary at
`build/bin/legmacs` (needs a [let-go](https://github.com/nooga/let-go)
source checkout beside this repo; override with `LETGO_DIR`).

Multiple file arguments each open in their own buffer (the first one
focused); see "Using it" below for switching between them.

The wrapper just sets `lg`'s namespace search path so `legmacs.*` resolves
regardless of your current directory, and so an optional
...
of `src/`.) `C-n`/`C-p` (or the arrows) move the highlight,
`TAB` pulls the highlighted candidate up into the input line (so you can
descend into a directory or refine further). `M-x` and the buffer/mode
pickers submit the **highlighted** candidate on `RET`; the file prompts
submit exactly the **path you typed** on `RET` (so a brand-new filename is
never silently swapped for a lookalike) — `TAB` then `RET` to open a listed
one, or `M-RET` anywhere to force the raw text.
...
command — built-in or from your config — can modify a read-only buffer,
while movement, scrolling, and search keep working.
...
one shared clipboard are not — they're exactly as visible switching buffers
as they would be switching windows in something that had windows.

`C-SPC` sets the mark, then move point anywhere — the region between the
...
— a mode can add a new leaf under an existing global prefix (let-go-mode
adds `C-x C-e` without hiding `C-x C-s`/`C-x C-c`/etc), and anything the
mode doesn't bind falls straight through to global. A mode can also plug
into rendering (`:highlighter`, a pure per-line syntax colorer) and into
every keystroke (`:after-command`, e.g. to keep a matched-paren highlight
current) — see [`legmacs/modes.lg`](legmacs/modes.lg)'s `register-mode!`.
...
closed" — the two look identical from the buffer's text alone until you
...
`dispatch/`, `render/`) plus all of `clojure.core`, live — e.g.
...
highlighters share — getting it exactly right needs the kind of
continuation state Emacs tracks via syntax-table text properties).
...
[`legmacs/modes/markdown.lg`](legmacs/modes/markdown.lg). Like let-go-mode's
highlighter, it's per-line with no state carried across lines, so only a
fenced-code-block's ` ``` ` delimiter lines are recognized, not the code
between them (the same limitation, for the same reason, as the multi-line
string case above).
...
already has — a `.lg` file keeps let-go-mode's highlighting, paren-matching,
...
straight through to the major mode / global self-insert — insert mode *is*
...
The mode line ([`legmacs/modeline.lg`](legmacs/modeline.lg)) is a registry of
named, ordered, independently-colored **segments** plus a small theme — the
...
`main.lg` — there's no global mutable buffer. The only genuinely global,
...
`legmacs.dispatch`/`render`/`commands` only ever see one flat state — that
...
- `C-x C-c` only *saves* the current buffer. Other modified buffers get a
  ...
  it won't save them for you — switch to them and save individually first
